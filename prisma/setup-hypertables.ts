...
async function setupHypertables() {
  ...
  try {
    ...
    await prisma.$executeRawUnsafe(`SELECT add_compression_policy('native_transfers', INTERVAL '7 days', if_not_exists => TRUE)`);
    await prisma.$executeRawUnsafe(`SELECT add_compression_policy('cex_flows', INTERVAL '7 days', if_not_exists => TRUE)`);
    await prisma.$executeRawUnsafe(`SELECT add_compression_policy('decoded_events', INTERVAL '7 days', if_not_exists => TRUE)`);
    await prisma.$executeRawUnsafe(`SELECT add_compression_policy('stablecoin_transfers', INTERVAL '7 days', if_not_exists => TRUE)`);
    await prisma.$executeRawUnsafe(`SELECT add_compression_policy('stablecoin_cex_flows', INTERVAL '7 days', if_not_exists => TRUE)`);
    await prisma.$executeRawUnsafe(`SELECT add_compression_policy('pool_swaps', INTERVAL '7 days', if_not_exists => TRUE)`);
    await prisma.$executeRawUnsafe(`SELECT add_compression_policy('pool_liquidity', INTERVAL '7 days', if_not_exists => TRUE)`);
    await prisma.$executeRawUnsafe(`SELECT add_compression_policy('pool_fees', INTERVAL '7 days', if_not_exists => TRUE)`);
